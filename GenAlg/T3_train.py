...
import random, csv, asyncio, ast

images = []
labels = []
with open("MinMax_data.csv", 'r') as file:
    csvreader = csv.reader(file)
    header = next(csvreader)
    for row in csvreader:
        if row:
            res = ast.literal_eval(row[0])
            res = [int(x[i]) for i in range(3) for x in res]
            images.append(res)
            labels.append(int(row[1]))

images_slice, labels_slice = images[:500], labels[:500]

print(images_slice[0])
...
ITS = 300
dimensions = [9, 20, 16, 9]
functions = ['relU', 'sigmoid', 'direct']
